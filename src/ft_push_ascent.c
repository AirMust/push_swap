...
void ft_push_ascent(t_ps *ps, char flag, int max, int current)
{
	...
	{
		while (++i < length)
			if (array[i] == (flag == 'b' ? ft_array_max(array, length) : ft_array_min(array, length)))
			{
		...
		while (++i < length - 1)
			if (flag == 'b' ? current < array[i] && current > array[i + 1] : current < array[i + 1] && current > array[i])
			{
...
void ft_push_prev_ascent(t_ps *ps, char flag)
{
	if (flag == 'a')
	{
		if (ps->b[0] < ft_array_min(ps->a, ps->lenA) || ps->b[0] > ft_array_max(ps->a, ps->lenA))
	...
	if (flag == 'b')
	{
		if (ps->a[0] < ft_array_min(ps->b, ps->lenB) || ps->a[0] > ft_array_max(ps->b, ps->lenB))
